...
def generate_attacker_matrix(REC_DATA, FRAME_ID):
    ADJ = np.zeros((40, 40))
    ...
    np.savetxt('../../output/csv/ADJ/ADJ_A_%s.csv' %(FRAME_ID.split(" ", 1)[0]), ADJ, fmt='%.4f', delimiter=',')
    print "Generated Attacker Sequence adjacency matrix for %s" %(FRAME_ID.split(" ", 1)[0])
    return None
